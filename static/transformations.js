...
export const transformations = {
    ...
    "7-Banco_del_TesoroAVIPRORCA": (data) => {
        const diario = "Banco del Tesoro";
        if (!Array.isArray(data) || data.length === 0) {
            return [['ETIQUETA', 'FECHA', 'REFERENCIA', 'IMPORTE', 'DIARIO']];
        }

        const result = JSON.parse(JSON.stringify(data));
        let headerRowIndex = result.findIndex(row => {
            if (!Array.isArray(row)) return false;
            const normalized = row.map(cell => typeof cell === 'string' ? cell.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') : '');
            return normalized.some(cell => cell.includes('fecha'))
                && normalized.some(cell => cell.includes('referencia'))
                && normalized.some(cell => cell.includes('concepto') || cell.includes('detalle') || cell.includes('movimiento'));
        });
        ...
        const header = result[headerRowIndex] || [];
        const rows = result.slice(headerRowIndex + 1);
        const headerNames = header.map(h => typeof h === 'string' ? h.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') : '');
        ...
        const transformed = rows.map((row) => {
            ...
            const referencia = row[idxReferencia] || '';
            const etiqueta = row[idxConcepto] || '';
        ...
        transformed.unshift(['ETIQUETA', 'FECHA', 'REFERENCIA', 'IMPORTE', 'DIARIO']);
